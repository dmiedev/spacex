    ...
    return BlocBuilder<LaunchFilteringBloc, LaunchFilteringState>(
      ...
      builder: (context, state) => ActionChip(
        avatar: Icon(
          Icons.rocket,
          size: _chipIconSize,
          color: state.rockets.isNotEmpty ? Colors.black : null,
        ),
        label: Text(
          state.rockets.isEmpty
              ? 'Rockets'
              : state.rockets.length == 1
                  ? state.allRockets![state.rockets.first].name
                  : 'Rockets: ${state.rockets.length}',
          style: state.rockets.isNotEmpty
              ? const TextStyle(color: Colors.black)
              : null,
        ),
        onPressed: () => _handlePress(context),
        backgroundColor: state.rockets.isNotEmpty ? Colors.white : null,
      ),
    );
  }
...
class _RocketSelectionDialogState extends State<_RocketSelectionDialog> {
  ...
  void _initializeRocketSelection(
    ...
    _rocketSelection =
        List<bool>.generate(rockets.length, selectedIndices.contains);
  }
